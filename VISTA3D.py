...
try:
    ...
    class SegResNetDS2(SegResNetDS):
        """
        Backward-compatible fallback for environments that provide SegResNetDS
        but not SegResNetDS2 (e.g. MONAI 1.3.x).
        """

        def forward(
            self,
            x: torch.Tensor,
            with_point: bool = True,  # noqa: ARG002
            with_label: bool = True,  # noqa: ARG002
...
try:
    ...
    def unsqueeze_right(x: torch.Tensor, ndim: int) -> torch.Tensor:
        while x.ndim < ndim:
            x = x.unsqueeze(-1)
        return x
...
try:
    ...
    def convert_points_to_disc(
        ...
        """Fallback for MONAI versions without `convert_points_to_disc`."""
        h, w, d = spatial_shape
        device = point_coords.device
        yy = torch.arange(h, device=device).view(1, 1, h, 1, 1)
        xx = torch.arange(w, device=device).view(1, 1, 1, w, 1)
        zz = torch.arange(d, device=device).view(1, 1, 1, 1, d)
        cy = point_coords[..., 0].round().long().view(point_coords.shape[0], point_coords.shape[1], 1, 1, 1)
        cx = point_coords[..., 1].round().long().view(point_coords.shape[0], point_coords.shape[1], 1, 1, 1)
        cz = point_coords[..., 2].round().long().view(point_coords.shape[0], point_coords.shape[1], 1, 1, 1)
        dist2 = (yy - cy) ** 2 + (xx - cx) ** 2 + (zz - cz) ** 2
        mask = dist2 <= int(radius) ** 2
        valid = (point_labels >= 0).view(point_labels.shape[0], point_labels.shape[1], 1, 1, 1)
        return (mask & valid).to(point_coords.dtype)
...
try:
    ...
    def lcc(
        ...
        point_coords: torch.Tensor,  # noqa: ARG001
        point_labels: torch.Tensor,  # noqa: ARG001
    ) -> torch.Tensor:
        """Fallback for MONAI versions without connected-component helper."""
        return torch.logical_or(diff_pos, diff_neg)
...
try:
    ...
    def sample_points_from_label(
        ...
        """Fallback point sampler used only for point-based validation paths."""
        if device is None:
            device = labels.device
        num_obj = len(label_set)
        num_points = max_ppoint + max_npoint
        point_coords = torch.zeros((num_obj, num_points, 3), dtype=torch.float32, device=device)
        point_labels = -torch.ones((num_obj, num_points), dtype=torch.long, device=device)
        label_map = labels[0, 0]

        for i, cls in enumerate(label_set):
            coords = torch.nonzero(label_map == int(cls), as_tuple=False)
            if coords.numel() == 0 or num_points == 0:
                continue
            if use_center:
                idx = coords.shape[0] // 2
            else:
                idx = int(torch.randint(0, coords.shape[0], (1,), device=device).item())
            point_coords[i, 0] = coords[idx].to(torch.float32)
            point_labels[i, 0] = 1
        return point_coords, point_labels

rearrange, _ = optional_import("einops", name="rearrange")

__all__ = ["VISTA3D", "vista3d132", "AortaVista3DModel", "build_aorta_vista3d_model"]
...
class TwoWayAttentionBlock(nn.Module):
    def __init__(
        ...
        try:
            ...
            # MONAI <=1.3 doesn't have "vista3d" dropout mode.
            self.mlp = MLPBlock(hidden_size=embedding_dim, mlp_dim=mlp_dim, act=activation, dropout_mode="vit")
...
class AortaVista3DModel(nn.Module):
    """
    Task adapter for aorta segmentation with explicit merged foreground output.

    MONAI VISTA3D's automatic branch predicts one foreground mask per class prompt.
    This wrapper runs class prompts for TL and FL, then returns four channels:
    background, class-1, class-2, and merged foreground (class-1 or class-2).
    """
    ...
    def __init__(
        ...
        super().__init__()
        if len(class_ids) != 2:
            raise ValueError(f"class_ids must contain exactly 2 foreground classes, got {class_ids}.")

        self.model = vista3d132(encoder_embed_dim=encoder_embed_dim, in_channels=in_channels)
        class_vector = torch.as_tensor(class_ids, dtype=torch.long).reshape(-1, 1)
        self.register_buffer("class_vector", class_vector, persistent=False)

    def forward(self, input_images: torch.Tensor) -> torch.Tensor:
        if input_images.ndim != 5:
            raise ValueError(f"input_images must have shape [B, C, H, W, D], got {tuple(input_images.shape)}.")

        batch_logits = []
        class_vector = self.class_vector.to(input_images.device)

        for image in input_images.split(1, dim=0):
            fg_logits = self.model(input_images=image, class_vector=class_vector)
            if fg_logits.ndim != 5 or fg_logits.shape[1] != 1:
                raise RuntimeError(
                    "Unexpected VISTA3D output shape. Expected [N_foreground, 1, H, W, D], "
                    f"got {tuple(fg_logits.shape)}."
                )

            fg_logits = fg_logits.squeeze(1)  # [N_foreground, H, W, D]
            bg_logits = torch.zeros(
                (1, *fg_logits.shape[1:]),
                device=fg_logits.device,
                dtype=fg_logits.dtype,
            )
            fg_binary_logit = torch.logsumexp(fg_logits, dim=0, keepdim=True) - bg_logits
            logits = torch.cat((bg_logits, fg_logits, fg_binary_logit), dim=0).unsqueeze(0)  # [1, 4, H, W, D]
            batch_logits.append(logits)

        return torch.cat(batch_logits, dim=0)


def build_aorta_vista3d_model(
    in_channels: int = 1,
    encoder_embed_dim: int = 48,
    class_ids: Sequence[int] = (1, 2),
) -> AortaVista3DModel:
    """Factory for the aorta-specific VISTA3D adapter."""
    return AortaVista3DModel(
        in_channels=in_channels,
